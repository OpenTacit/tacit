...
![The Review page shows the queue counts, and the drafts table with its selection checkboxes and the Suggest candidate techniques button](../images/review-queue.png)
...
evidence. The registry keeps a rejected draft. It does not erase it. You
can remove drafts fully, for example spam or the results of a bad research
pass. Select their checkboxes in the list. Then click **Delete selected**.
You must confirm the deletion. Deletion applies only to drafts. You cannot
undo a deletion.
