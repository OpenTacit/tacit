...
| `TACIT_PUBLISH_TEST` | Declares this registry a test rather than an organization's, so the proxy's operator can tell the two apart and may reclaim its hostname once it stops connecting. Set it on a scratch registry, a trial, or a CI run. Default off, except under `go test`, where it is on. |
...
If the registry is a scratch one — a trial, a CI run, anything exercising the
proxy rather than serving an organization — say so as well:

```bash
TACIT_PUBLISH_TEST=1
```

The proxy then marks it as a test in its console and may reclaim its hostname
once it stops connecting, instead of holding the name forever. Nothing else
about the connection changes. A registry built by `go test` sets this on its
own, so the case that needs the line is one you start by hand.
...
acceptable, you have two other options: keep the function off and put
[your own reverse proxy](#serve-under-a-sub-path) in front of the registry, or
[run a proxy of your own](#run-your-own-proxy). The other parts of the registry
stay the same in every case.
...
The proxy is part of OpenTacit. Nothing about it is reserved for the
shared one, so you can run the same program yourself and keep every
request inside your own network:

```bash
tacit-ingress serve --zone tacit.example.com \
  --tls-cert /etc/tls/wildcard.pem --tls-key /etc/tls/wildcard.key
```

It needs three things: a wildcard DNS record for the zone, because each
registry gets a name under it; a certificate for that wildcard; and a port
that browsers and registries can both reach. A registry pointed at a URL
opens its tunnel on that same port. `tacit-ingress serve --print-config`
prints the settings it resolved, and exits.

Point a registry at it with one line in `registry.env`, then restart:

```bash
TACIT_PUBLISH_INGRESS=https://ingress.tacit.example.com
```

The registry enrols itself and is given a name, exactly as it would be on
the shared proxy. Read the route table on the host that runs the proxy:

```bash
tacit-ingress instances list        # every name, and when it was last seen
tacit-ingress instances rm <name>   # release a name
```

There is no operator console in this program, and the address it answers
on shows a status page that names no registry. The route table is for the
person at the host, which is the same rule the rest of OpenTacit follows:
the operator of a proxy sees the envelope, and never the contents.
