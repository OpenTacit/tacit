...
2. **Search the registry for each activity.** Prefer the `tacit_search` tool
   (from the `tacit` MCP server this package wires). If it is unavailable or
   denied, fall back to the registry API directly:
