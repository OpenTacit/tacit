...
breakdown, for the window **whatever the member wrote after the "tacit usage" request** (default `30d`; valid: 7d, 30d, 90d, all).
...
1. Show the text summary inline — this renders in every harness:
   ...
   Substitute `<window>` with **whatever the member wrote after the "tacit usage" request** (omit `--window` for the `30d`
