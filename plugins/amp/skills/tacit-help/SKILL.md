---
name: tacit-help
description: "Explain Tacit features and commands. Use when the member invokes $tacit-help or asks how Tacit works."
---
...
- **Statusline**: `◆ tacit N⚡ M✓ K⚑` shows **N⚡** suggestions
  ...
  with $tacit-drafts. It also carries a warning when a fault has made Tacit
...
- $tacit-search `<what you're trying to do>`: ask whether the org has
  a validated way to do something. Do this before inventing an approach that
  internal tools or conventions might cover.
- $tacit-review: check the work in flight against the registry and surface
...
- $tacit-contribute: capture a reusable approach as a draft technique. Drafts are
  excluded from retrieval until a reviewer promotes them.

## Other commands

- $tacit-drafts: the review queue (promote/reject/edit).
- $tacit-setup `[url]`: configure and verify the registry URL and key.
- $tacit-suggest `[count]`: research techniques matched to observed usage; files up to 10 drafts.
- $tacit-insights `[7d|30d|90d|all]`: technique-usage metrics (funnel, helped rate).
- $tacit-org `[7d|30d|90d|all]`: how the org uses its playbook — cohort spread, opportunities, proponents.
- $tacit-status: stack health; $tacit-test: end-to-end pipeline
  check with a paste-ready trigger prompt; $tacit-testfeedback: deliver one
