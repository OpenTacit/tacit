...
breakdown, for the window **what the member asked for, in plain words** (default `30d`; valid: 7d, 30d, 90d, all).
...
1. Show the text summary inline — this renders in every harness:
   ...
   Substitute `<window>` with **what the member asked for, in plain words** (omit `--window` for the `30d`
