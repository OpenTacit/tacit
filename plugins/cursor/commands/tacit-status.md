...
Report the health of the member's Tacit integration (registry, hook daemon, model, MCP tool). Use when the member invokes /tacit-status or asks whether Tacit is up/working.
...
Add `--harness whatever the member wrote after the /tacit-status command` when the member names one, or when suggestions have
