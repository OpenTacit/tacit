...
Capture a reusable approach from the current session as a draft Tacit technique. Use when the member says things like "contribute this to Tacit", "capture this as a technique", "add this move to the registry", or invokes /tacit-contribute. Confirms the structured fields with the harness's native picker or a compact numbered form in chat, then submits for review.
