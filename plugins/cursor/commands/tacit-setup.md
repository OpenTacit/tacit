...
Wire this member's harness to the org Tacit registry: prompt for the registry URL and API key, save them to ~/.config/tacit/agent.env via `tacit connect --settings-only`, and verify. Use when a member asks to connect to Tacit, set the registry URL/key, or onboard this machine.
...
Suggest /tacit-status (stack health) and /tacit-test (full pipeline).
