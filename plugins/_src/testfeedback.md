+++
slug = testfeedback
short = Prove Tacit reaches you: one ◆ block through the hook channel, and one native suggestion form
long = Arm a delivery self-test so one ◆ Tacit block arrives through the real hook channel at the end of this turn, proving the member would see a live suggestion. Use when the member invokes {{.Idiom}} or asks whether Tacit's feedback actually reaches them.
allowed_tools = Bash(tacit:*){{if .Picker}}, {{.Picker}}{{end}}
+++
Prove that a Tacit suggestion is VISIBLE in this session.

{{cmd "status"}} says the pieces are running and {{cmd "test"}} says retrieval
...
**{{.Picker}}**, still inside this turn, before you finish — header, question
...
- Say the ◆ block should follow your reply. If it does not, that is the
  ...
  and {{cmd "review"}} pulls suggestions into ordinary conversation instead.
{{else}}
Then stop. Say, in two lines at most, that the block should appear right below
your reply, and that seeing it means live suggestions will reach them here.
{{end}}
## Rules
...
{{- else}}
- End the turn as soon as the command returns. No other tools, no follow-up
  work: the block rides the end of the turn.
{{- end}}
...
- `delivered` — the agent returned the block and the client dropped it. Claude
  ...
  suggestions, pull: ask for a technique, or run {{cmd "review"}}.
