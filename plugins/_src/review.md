+++
slug = review
short = Review work in flight for relevant techniques from the org's playbook and render them inline
long = Review current work for relevant techniques from the org's Tacit playbook. Use when the member invokes {{.Idiom}} or asks to review work against Tacit.
arg_hint = optional focus (blank = whatever you're working on now)
allowed_tools = Bash(curl:*), Bash(tacit:*), {{tool "search"}}
+++
{{preamble}}
...
of one family that all run the same playbook retrieval: {{cmd "search"}} (a query
you name), this (work in flight), and {{cmd "audit"}} (a finished session, which
...
Prefer the `{{tool "search"}}` MCP tool. If it's unavailable or denied, fall
...
to capture it with {{cmd "contribute"}}. Record outcomes only with the member's
