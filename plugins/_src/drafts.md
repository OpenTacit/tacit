+++
slug = drafts
short = Review Tacit drafts awaiting decision — promote, reject, or edit, without the dashboard
long = Review the Tacit draft queue — promote, reject, or edit draft techniques without the dashboard. Use when the member invokes {{.Idiom}} or asks what's waiting for Tacit review.
...
Call the `{{tool "drafts"}}` MCP tool (load it via ToolSearch if deferred). It
...
- **If this client renders the panel** (the member sees the drafts with
  ...
  `{{tool "draft_action"}}` themselves; do not duplicate its work in chat.
...
Prefer the `{{tool "draft_action"}}` MCP tool: arguments `{id, action}` with
