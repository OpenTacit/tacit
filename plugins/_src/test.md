+++
slug = test
short = End-to-end test of the Tacit pipeline (retrieval, hooks, trigger prompt)
long = Run an end-to-end test of the Tacit pipeline (registry, retrieval, hooks) and produce a paste-ready trigger prompt. Use when the member invokes {{.Idiom}} or asks to verify Tacit works.
arg_hint = optional topic to test with
allowed_tools = Bash(curl:*)
+++
{{preamble}}

Run an end-to-end test of the Tacit pipeline and report PASS/FAIL per stage.
Optional focus topic: {{.ArgRef}}
...
5. **In-flow trigger prompt**: compose a short, realistic user prompt that
   ...
   result is ambiguous — {{cmd "testfeedback"}} tests delivery alone and always
   fires.
...
command to fix it before suggesting the member re-run {{cmd "test"}}.
