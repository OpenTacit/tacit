+++
slug = usage
short = Your OWN Tacit usage — queries and suggestions shown/adopted/helped over time, from this machine's local log; personal, never leaves your machine
long = Show the current member's OWN Tacit usage — their queries, and suggestions shown, adopted, and measurably helped over time, with a per-technique breakdown — read from this machine's local log. Use when the member invokes {{.Idiom}} or asks "how am I using Tacit". Personal and machine-local, never the org's cohort aggregates (for those use {{cmd "insights"}}).
arg_hint = 7d | 30d | 90d | all (default 30d)
allowed_tools = Bash(tacit usage:*), Bash(tacit env:*)
+++
{{preamble}}
...
breakdown, for the window **{{.ArgRef}}** (default `30d`; valid: 7d, 30d, 90d, all).
...
1. Show the text summary inline — this renders in every harness:
   ...
   Substitute `<window>` with **{{.ArgRef}}** (omit `--window` for the `30d`
