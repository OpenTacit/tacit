+++
slug = help
short = Explain Tacit features and commands
long = Explain Tacit features and commands. Use when the member invokes {{.Idiom}} or asks how Tacit works.
+++
{{preamble}}
...
- **Statusline**: `◆ tacit N⚡ M✓ K⚑` shows **N⚡** suggestions
  ...
  with {{cmd "drafts"}}. It also carries a warning when a fault has made Tacit
...
- {{cmd "search"}} `<what you're trying to do>`: ask whether the org has
  a validated way to do something. Do this before inventing an approach that
  internal tools or conventions might cover.
- {{cmd "review"}}: check the work in flight against the registry and surface
...
- {{cmd "contribute"}}: capture a reusable approach as a draft technique. Drafts are
  excluded from retrieval until a reviewer promotes them.

## Other commands

- {{cmd "drafts"}}: the review queue (promote/reject/edit).
- {{cmd "setup"}} `[url]`: configure and verify the registry URL and key.
- {{cmd "suggest"}} `[count]`: research techniques matched to observed usage; files up to 10 drafts.
- {{cmd "insights"}} `[7d|30d|90d|all]`: technique-usage metrics (funnel, helped rate).
- {{cmd "org"}} `[7d|30d|90d|all]`: how the org uses its playbook — cohort spread, opportunities, proponents.
- {{cmd "status"}}: stack health; {{cmd "test"}}: end-to-end pipeline
  check with a paste-ready trigger prompt; {{cmd "testfeedback"}}: deliver one
