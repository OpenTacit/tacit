+++
slug = suggest
short = Research and draft techniques matched to this org's Tacit usage
long = Research and draft techniques matched to this org's observed Tacit usage: registry-side when it has an Anthropic key, otherwise via the harness's own web search. Use when the member invokes {{.Idiom}} or asks Tacit to suggest techniques.
arg_hint = optional count, up to 10 (default 10)
allowed_tools = Bash(curl:*), Bash(tacit:*), WebSearch, WebFetch
+++
{{preamble}}
