+++
slug = status
short = Health check of the Tacit stack (registry, hooks, model, MCP)
long = Report the health of the member's Tacit integration (registry, hook daemon, model, MCP tool). Use when the member invokes {{.Idiom}} or asks whether Tacit is up/working.
arg_hint = optional harness name for an end-to-end hop check
allowed_tools = Bash(tacit:*)
+++
...
Add `--harness {{.ArgRef}}` when the member names one, or when suggestions have
...
list for `{{tool "search"}}` (load it via ToolSearch if deferred); registered
...
For the org's numbers rather than this machine's health, use {{cmd "insights"}}.
