+++
slug = audit
short = Retrospective Tacit audit of an agent session: review what happened and follow up
long = Retrospective Tacit audit of an agent session: replay a finished session (transcript, share URL, or the current one) against the org's Tacit playbook and follow up with feedback, contributions, and revisions. Use when the member invokes {{.Idiom}} or asks to audit a session.
arg_hint = transcript file or share URL (blank = this session)
allowed_tools = Bash(tacit:*), Bash(curl:*)
+++
{{preamble}}
...
playbook retrieval: {{cmd "search"}} (a query you name), {{cmd "review"}} (work in
...
2. **Recurring uncovered friction**: something the member
   ...
   it with {{cmd "contribute"}}, naming the move.
