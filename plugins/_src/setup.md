+++
slug = setup
short = Wire this member's harness to the org Tacit registry: prompt for URL + key, save, verify
long = Wire this member's harness to the org Tacit registry: prompt for the registry URL and API key, save them to ~/.config/tacit/agent.env via `tacit connect --settings-only`, and verify. Use when a member asks to connect to Tacit, set the registry URL/key, or onboard this machine.
arg_hint = optional registry URL
...
Suggest {{cmd "status"}} (stack health) and {{cmd "test"}} (full pipeline).
