---
description: Your OWN Tacit usage — queries and suggestions shown/adopted/helped over time, from this machine's local log; personal, never leaves your machine
argument-hint: 7d | 30d | 90d | all (default 30d)
allowed-tools: Bash(tacit usage:*), Bash(tacit env:*)
---
...
breakdown, for the window **$ARGUMENTS** (default `30d`; valid: 7d, 30d, 90d, all).
...
1. Show the text summary inline — this renders in every harness:
   ...
   Substitute `<window>` with **$ARGUMENTS** (omit `--window` for the `30d`
