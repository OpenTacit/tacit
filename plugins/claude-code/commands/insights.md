---
description: Tacit technique-usage metrics (funnel, helped rate) as a dashboard with text fallback — for who/where questions use /tacit:org
...
Show the organization's Tacit technique-usage insights for the window **$ARGUMENTS**
...
1. Call the `mcp__plugin_tacit_tacit__tacit_metrics` MCP tool (from the `tacit` MCP server) with
