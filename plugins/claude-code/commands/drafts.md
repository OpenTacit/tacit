...
Call the `mcp__plugin_tacit_tacit__tacit_drafts` MCP tool (load it via ToolSearch if deferred). It
...
- **If this client renders the panel** (the member sees the drafts with
  ...
  `mcp__plugin_tacit_tacit__tacit_draft_action` themselves; do not duplicate its work in chat.
...
Prefer the `mcp__plugin_tacit_tacit__tacit_draft_action` MCP tool: arguments `{id, action}` with
