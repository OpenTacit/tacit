---
description: Review work in flight for relevant techniques from the org's playbook and render them inline
argument-hint: optional focus (blank = whatever you're working on now)
allowed-tools: Bash(curl:*), Bash(tacit:*), mcp__plugin_tacit_tacit__tacit_search
---
...
of one family that all run the same playbook retrieval: /tacit:search (a query
you name), this (work in flight), and /tacit:audit (a finished session, which
...
Prefer the `mcp__plugin_tacit_tacit__tacit_search` MCP tool. If it's unavailable or denied, fall
...
to capture it with /tacit:contribute. Record outcomes only with the member's
