---
name: tacit-status
description: Report the health of the member's Tacit integration (registry, hook daemon, model, MCP tool). Use when the member invokes /skill:tacit-status or asks whether Tacit is up/working.
---
...
Add `--harness whatever the member wrote after the /skill:tacit-status command` when the member names one, or when suggestions have
...
list for `tacit_search` (load it via ToolSearch if deferred); registered
...
For the org's numbers rather than this machine's health, use /skill:tacit-insights.
