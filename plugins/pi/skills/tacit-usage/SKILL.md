---
name: tacit-usage
description: Show the current member's OWN Tacit usage — their queries, and suggestions shown, adopted, and measurably helped over time, with a per-technique breakdown — read from this machine's local log. Use when the member invokes /skill:tacit-usage or asks "how am I using Tacit". Personal and machine-local, never the org's cohort aggregates (for those use /skill:tacit-insights).
---
...
breakdown, for the window **whatever the member wrote after the /skill:tacit-usage command** (default `30d`; valid: 7d, 30d, 90d, all).
...
1. Show the text summary inline — this renders in every harness:
   ...
   Substitute `<window>` with **whatever the member wrote after the /skill:tacit-usage command** (omit `--window` for the `30d`
