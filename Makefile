...
.PHONY: all build build-onnx install install-ingress ingress dist-ingress deploy-ingress image deploy site plugins prices test test-go test-plugins test-pg licenses licenses-report vet fmt clean
...
## install-ingress: install the ingress binary (docs/distribution/ingress.md).
...
## deploy-ingress: build, copy and restart the ingress on the server.
## INGRESS_HOST is an ssh destination — an alias in ~/.ssh/config keeps the key
## and user out of here.
##
## The copy lands on a temporary name and is moved into place: writing directly
## over a running executable fails with ETXTBSY, and a half-copied binary that
## systemd then restarts is worse than a failed scp.
INGRESS_HOST ?= ingress
deploy-ingress: dist-ingress
	scp dist/tacit-ingress-linux-$(ARCH) $(INGRESS_HOST):/usr/local/bin/tacit-ingress.new
	ssh $(INGRESS_HOST) 'set -e; \
		chmod 755 /usr/local/bin/tacit-ingress.new; \
		mv /usr/local/bin/tacit-ingress.new /usr/local/bin/tacit-ingress; \
		systemctl restart tacit-ingress; \
		sleep 1; \
		systemctl is-active --quiet tacit-ingress || { journalctl -u tacit-ingress -n 20 --no-pager; exit 1; }; \
		/usr/local/bin/tacit-ingress version'
	@echo "deployed to $(INGRESS_HOST)"

## ingress: run a local ingress for testing — console on http://localhost:8443,
## tunnel on :8444, data under ./ingress-data, no sign-in. See
## docs/distribution/ingress.md for the five-minute end-to-end run.
