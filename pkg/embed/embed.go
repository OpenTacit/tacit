...
// MoveText is the text that says what a technique DOES, for deciding whether
// two techniques are the same one. The recipe, and nothing else.
//
// It is deliberately not TechniqueText. That composes what a technique applies
// TO — name, description, triggers, applicability, tags — because retrieval
// matches it against a member's situation, and it leaves the recipe out for
// that reason. Sameness is a different question, asked of the move itself, and
// asking it of the applicability text is how a registry ends up holding fifty
// differently-worded copies of one instruction.
//
// The name is left out for the same reason the description is: it is the part
// that varies between restatements, so including it argues that two techniques
// differ because somebody named them differently. Measured over a 174-technique
// corpus, replaying every arrival through the matcher: the recipe alone turned
// away 47, name-and-recipe 43, and every one of the 47 was a genuine restatement
// — 28 of them of a single technique.
func MoveText(technique contracts.Technique) string {
	return strings.TrimSpace(technique.Recipe)
}
