...
type Config struct {
	...
	// TestRetentionDays releases the name of a TEST instance that has not
	// connected for this many days; 0 leaves every name alone forever.
	//
	// Only instances that declared themselves tests are ever swept (protocol.go,
	// Hello.Test). A registry that has said nothing about itself keeps its
	// hostname however long it has been away — coming back after a quiet
	// quarter to find your address reissued is the failure this service exists
	// to prevent, and no retention figure is worth making that reachable by
	// inference.
	TestRetentionDays int
...
func FromEnv() Config {
	c := Config{
		...
		// Off by default. Reclaiming a name is not something an ingress should
		// start doing because somebody upgraded it.
		TestRetentionDays: envInt("TACIT_INGRESS_TEST_RETENTION_DAYS", 0),
...
// WithDefaults fills in what a configuration left unsaid. FromEnv and New both
// apply it, so it is only needed by something that builds a Config by hand and
// then asks it a question — what the callbacks are, which host is the apex —
