...
type Hello struct {
	...
	// Test marks a connection made to exercise the proxy rather than to serve
	// an organization. The registry is the only thing that can honestly say so
	// — the proxy sees a handshake and a hostname and cannot tell a trial run
	// from a tenant's first day — so it says it here, on every connect, and the
	// proxy records what it was told. A registry that stops saying it stops
	// being one.
	//
	// What it buys is a name the ingress may reclaim: a test instance that goes
	// quiet is released on its own (Config.TestRetentionDays), and a tenant's
	// is not, however long it has been away. Nothing else about the connection
	// changes — a test instance is routed, rate-limited and logged exactly like
	// any other, because a proxy that treated them differently would not be
	// testing the proxy.
