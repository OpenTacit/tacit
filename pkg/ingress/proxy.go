...
func (s *Server) PublicHandler() http.Handler {
	// No console mounted means this is a proxy somebody runs for themselves, and
	// the status page is the whole of what it says about itself (status.go). The
	// console that manages the project's own instance mounts itself through
	// Server.Console (operator.go).
	console := s.Console
	if console == nil {
		console = s.statusHandler()
	}
	// Likewise the product's domain and the zone's apex: with nothing serving a
	// page there, they are hostnames this proxy answers on and nothing more.
	site := s.Site
	if site == nil {
		site = console
	}
