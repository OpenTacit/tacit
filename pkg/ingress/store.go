...
type Instance struct {
	...
	// Test is what the registry said about itself at its last handshake
	// (protocol.go, Hello.Test): a connection made to exercise the proxy rather
	// than to serve an organization. Reported, never inferred — the proxy
	// cannot tell a trial run from a tenant's first day, and guessing would put
	// a real hostname within reach of a sweep.
	//
	// It is refreshed on every connect, so a registry that stops declaring it
	// stops being one, and its name stops being reclaimable.
...
// SetDisabledMany flips the kill switch on a set of instances under one lock
// and one save, returning the names it actually changed.
//
// It exists because the one-at-a-time path rewrites the whole route table per
// name. That is right for one decision and wrong for a thousand: a console
// suspending everything a filter matched was paying a megabyte of file rewrite
// per instance and holding the write lock for minutes to do what is one write.
// A name that is not here is skipped rather than refused — a bulk decision is
// taken against a list that was drawn a moment ago, and one of them going in
// between is ordinary, not an error.
func (s *Store) SetDisabledMany(names []string, disabled bool) ([]string, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
// DeleteMany drops a set of names under one lock and one save, for the reason
// SetDisabledMany gives. The names it returns are the ones that were there.
func (s *Store) DeleteMany(names []string) ([]string, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
// Touch records that an instance handshook, with what it said about itself.
//
// Test is taken from every handshake rather than only from the first, because
// it is a claim about what this registry currently is. A test instance rebuilt
// as a real one says so on its next connect and keeps its name from then on.
func (s *Store) Touch(name, version, source string, test bool) {
	s.mu.Lock()
	defer s.mu.Unlock()
	in, ok := s.byID[name]
	if !ok {
...
// ExpireTests releases the names of test instances last seen before the given
// time, and returns them. Nothing else is ever swept: an instance that has not
// declared itself a test keeps its hostname however long it has been away,
// because a registry coming back after a quiet quarter to find its address
// reissued is the one failure this whole service exists to prevent.
//
// A suspended instance is left alone too. Suspension is a decision somebody
// took about that name, and a sweep quietly undoing it would make the operator
// wrong about what they did.
func (s *Store) ExpireTests(before time.Time) ([]string, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	var dropped []string
	for name, in := range s.byID {
		if !in.Test || in.Disabled {
			continue
		}
		// A test instance that has never been seen is one that enrolled and
		// never came back; its enrolment stamp is the last thing known about it.
		last := in.LastSeen
		if last.IsZero() {
			last = in.Created
		}
...
func (s *Store) SetStats(name string, st Stats, windows bool) {
	s.SetStatsAt(name, st, windows, time.Now().UTC())
}

// SetStatsAt is SetStats at a stated instant. The live path passes the clock;
// anything reconstructing a history passes the day it is reconstructing, which
// is the only way a report can be older than the process reading it.
func (s *Store) SetStatsAt(name string, st Stats, windows bool, at time.Time) {
	s.mu.Lock()
	defer s.mu.Unlock()
	in, ok := s.byID[name]
	if !ok {
...
// SetDaySample records one day's figures for an instance, replacing that day's
// sample if there already is one and keeping the series in date order. SetStats
// is the live path and writes today from the tunnel; this is the only other way
// into the trend, for restoring a series or seeding one — a console's test of
// what two days of samples draw has no other way to have a yesterday.
func (s *Store) SetDaySample(name string, sample DaySample) {
	s.mu.Lock()
	defer s.mu.Unlock()
	in, ok := s.byID[name]
	if !ok {
	...
	for i, d := range in.Days {
		switch {
		case d.Day == sample.Day:
			in.Days[i] = sample
			s.markDirtyLocked()
			return
		case d.Day > sample.Day:
			in.Days = append(in.Days[:i], append([]DaySample{sample}, in.Days[i:]...)...)
			s.markDirtyLocked()
			return
		}
	}
