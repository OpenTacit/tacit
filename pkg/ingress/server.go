...
type Server struct {
	...
	// Console answers the console hostnames, and Site the product's own domain
	// and the zone's apex. Both are seams for a console that lives outside this
	// module (operator.go); nil means the built-in status page (status.go),
	// which reports that the proxy is up and names no instance.
...
type tunnel struct {
	...
	connected time.Time
	version   string
	// test is what this registry said about itself at the handshake
	// (Hello.Test). Set once at construction and never written again, like
	// remote, so it needs no lock.
	test bool
...
func (s *Server) serveTunnel(c net.Conn, br *bufio.Reader, remote string) {
	...
	switch hello.Role {
	case RoleControl:
		s.runControl(c, br, in, hello, remote)
	case RoleData:
...
func (s *Server) runControl(c net.Conn, br *bufio.Reader, in Instance, hello Hello, remote string) {
	t := s.newTunnel(c, in, hello, remote)
	s.installTunnel(t)
...
func (s *Server) newTunnel(c net.Conn, in Instance, hello Hello, remote string) *tunnel {
	t := &tunnel{
		name:      in.Name,
		instance:  in,
		connected: time.Now(),
...
func (s *Server) installTunnel(t *tunnel) {
	...
	s.Store.Touch(t.name, t.version, t.remote, t.test)
