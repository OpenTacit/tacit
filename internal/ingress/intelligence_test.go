...
func TestIntelligenceConsoleShowsOnlyThresholdedCrossOrgResults(t *testing.T) {
	h := newHarness(t)
	origin, _ := feed.LoadOrCreateKey(t.TempDir() + "/origin")
	originPublic := feed.PublicKeyString(origin.Public().(ed25519.PublicKey))
	otherOrigin, _ := feed.LoadOrCreateKey(t.TempDir() + "/other-origin")
	otherPublic := feed.PublicKeyString(otherOrigin.Public().(ed25519.PublicKey))

	for i := 1; i <= intel.CrossOrgMinRegistries; i++ {
		reporterKey, _ := feed.LoadOrCreateKey(fmt.Sprintf("%s/reporter-%d", t.TempDir(), i))
		report := intelligenceFixtureWithKey(i, originPublic, reporterKey, "2026-08-25T12:00:00Z")
		if i == 1 {
			other := report.Imports[0]
			other.Origin.ProviderID = "https://other.example"
			other.Origin.ProviderKey = otherPublic
			other.Origin.EntryID = "https://other.example/techniques/other-move"
			other.Origin.ContentHash = "sha256:1111111111111111111111111111111111111111111111111111111111111111"
			other.Outcome = &intel.Outcome{WindowStart: "2026-08-01T00:00:00Z", WindowEnd: "2026-08-25T12:00:00Z",
				Shown: 991, Adopted: 987, Helped: 654, SampleSize: 987}
			report.Imports = append(report.Imports, other)
			intel.Sign(&report, reporterKey)
		}
		if _, err := h.srv.Intel.Accept(fmt.Sprintf("private-instance-%d", i), report); err != nil {
			t.Fatal(err)
		}
	}

	page := getIntelligencePage(t, h, "/intelligence")
	for _, want := range []string{
		"Intelligence", "Outcome-ready versions", "Sharing registries", "Importing registries",
		"Cross-org adoptions", "Publishers", "Technique versions", "origin.example", "other.example",
		"60", "50", "83%", "held until 5 registries", "window.tacitSortableTable",
	} {
		if !strings.Contains(page, want) {
			t.Errorf("intelligence page is missing %q", want)
		}
	}
	// WHAT A READER CAN SEE, not what the document contains. The head carries
	// fingerprinted asset URLs — /assets/app.css?v=<content hash> — and a hash
	// is thirty-two hex characters that will sooner or later contain any given
	// run of digits. It did: an unrelated stylesheet edit moved the hash to
	// 52c237987361… and this test reported that the page had leaked a held
	// sample size of 987. No held figure is ever rendered in the head, so the
	// scan starts after it.
	body := page
	if i := strings.Index(page, "</head>"); i >= 0 {
		body = page[i:]
	}
	for _, private := range []string{"private-instance-1", "reporter-1.example", "987", "654"} {
		if strings.Contains(body, private) {
			t.Errorf("intelligence page exposed held or identifying value %q", private)
		}
	}

	waiting := getIntelligencePage(t, h, "/intelligence?state=waiting")
	if !strings.Contains(waiting, "other-move") || strings.Contains(waiting, `<code>move</code>`) {
		t.Fatalf("outcome-state filter did not scope technique versions")
	}
	if !strings.Contains(waiting, `<option value="waiting" selected>`) {
		t.Fatal("outcome-state filter did not survive in the URL-backed control")
	}
}

func getIntelligencePage(t *testing.T, h *harness, path string) string {
	t.Helper()
