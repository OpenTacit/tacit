...
func TestBulkDraftActionsDraftsOnly(t *testing.T) {
	srv, ts := newServer(t)
	// One draft per decision, so each is applied to a draft nothing else touched.
	// Each needs a move of its own: four contributions with one recipe between
	// them are one technique and three refusals.
	for _, n := range []string{"a", "b", "c", "d"} {
		resp, _ := request(t, "POST", ts.URL+"/v1/contribute", "test-key",
			`{"name": "bulk-`+n+`", "description": "d", "recipe": "recipe for `+n+`", "scope": "org"}`)
	...
	// The page carries the selection UI, and the bulk decisions are named and
	// wired the way the draft page names them.
	_, page := fetchHTML(t, ts.URL+"/review")
	if !strings.Contains(page, `form="bulk-drafts"`) {
		t.Fatal("drafts page lacks the bulk selection controls")
	}
	for _, want := range []string{
		`formaction="/admin/techniques/bulk/promote"`, ">Accept<",
		`formaction="/admin/techniques/bulk/to-shadow"`, ">Shadow<",
		`formaction="/admin/techniques/bulk/reject"`, ">Reject<",
		`formaction="/admin/techniques/bulk/delete"`, ">Delete selected<",
		// The behaviour behind the boxes is the shell's, shared with the
		// ingress console's fleet board. This page declares the markup and
		// writes no script: the header box, the live count, and the confirm
		// on the one decision that keeps nothing.
		`<input type="checkbox" form="bulk-drafts" data-select-all`,
		`data-count-for="bulk-drafts"`,
		`data-confirm="Delete {n} draft{s}?`,
	} {
		if !strings.Contains(page, want) {
			t.Errorf("drafts toolbar missing %q", want)
		}
	}
	// The selection script is the shell's one copy. A page growing its own
	// again is the drift this replaced.
	if strings.Contains(page, `document.getElementById('sel-all')`) {
		t.Error("the review lane grew a second copy of the selection script")
	}
	...
	// Each decision lands its drafts in the status the draft page's own button
	// lands one in, and delete is the one that keeps nothing. Every request
	// smuggles the stable id in alongside: the draft-only rule must hold
	// server-side, not just in the rendered UI.
	for i, tc := range []struct{ action, want string }{
		{"promote", "stable"},
		{"to-shadow", "shadow"},
		{"reject", "retired"},
		{"delete", ""},
		...
		id := drafts[i].ID
		postBulk(t, ts, tc.action, id, stable[0].ID)
		after, found, _ := srv.Store.GetTechnique(id)
		switch {
		case tc.want == "" && found:
			t.Errorf("bulk delete kept the draft (status %q)", after.Status)
		case tc.want != "" && (!found || after.Status != tc.want):
			t.Errorf("bulk %s: found %v status %q, want %q", tc.action, found, after.Status, tc.want)
		}
		if c, found, _ := srv.Store.GetTechnique(stable[0].ID); !found || c.Status != "stable" {
			t.Errorf("bulk %s touched a stable technique (found %v status %q)", tc.action, found, c.Status)
		}
	}

	// An action outside the four is not a lifecycle the review lane offers.
	req, _ := http.NewRequest("POST", ts.URL+"/admin/techniques/bulk/restore", nil)
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		t.Fatal(err)
	}
	...
	if resp.StatusCode != 404 {
		t.Errorf("unknown bulk action: %d, want 404", resp.StatusCode)
	}
}

// postBulk applies one bulk decision to the given ids, as the toolbar's form does.
func postBulk(t *testing.T, ts *httptest.Server, action string, ids ...string) {
	t.Helper()
	form := url.Values{"id": ids}
	req, _ := http.NewRequest("POST", ts.URL+"/admin/techniques/bulk/"+action, strings.NewReader(form.Encode()))
...
func TestBreadcrumbNavigationAndNestedDrafts(t *testing.T) {
	...
	// The leaf is the technique's own name. Read it from the file the server
	// loaded: a copy frozen here breaks when the technique is reworded, which
	// says nothing about breadcrumbs.
	loaded, err := techniques.ParseFile(filepath.Join("..", "..", "..", "techniques", "use-internal-data-connector.md"))
	if err != nil {
		t.Fatal(err)
	}
	for _, want := range []string{
		...
		`<span class="crumb-current" aria-current="page">` + loaded.Name + `</span>`,
...
func TestPublicUserGuideWhenOIDCOn(t *testing.T) {
	...
	if !strings.Contains(guide, "OpenTacit helps you use AI better") {
...
func TestDashboardPages(t *testing.T) {
	...
	// The id is in the URL, so finding it in the page proves nothing. The name
	// comes from the file the server loaded: present means the detail actually
	// rendered the technique, and reading it from disk survives a rewording.
	loaded, err := techniques.ParseFile(filepath.Join("..", "..", "..", "techniques", "use-internal-data-connector.md"))
	if err != nil {
		t.Fatal(err)
	}
	code, html = fetchHTML(t, ts.URL+"/techniques/use-internal-data-connector")
	if code != 200 || !strings.Contains(html, loaded.Name) {
		t.Fatalf("technique detail did not render %q: %d", loaded.Name, code)
	}
...
func TestHelpedRateLeaderboardMatchesDescendingTableTieOrder(t *testing.T) {
	...
	largerSample := strings.Index(leaderboard, "ask-for-a-diagram")
	smallerSample := strings.Index(leaderboard, "use-internal-data-connector")
