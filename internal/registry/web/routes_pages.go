...
func (s *Server) mountPageRoutes(mux *http.ServeMux) {
	...
	// {lane} is playbook or queue: the serving library's duplicates and the
	// evaluation queue's are separate decisions, taken on different evidence,
	// on the pages they belong to.
	mux.HandleFunc("POST /admin/techniques/consolidate/{lane}/propose", s.adminOnly(s.handleConsolidatePropose))
	mux.HandleFunc("POST /admin/techniques/consolidate/{lane}/apply", s.adminOnly(s.handleConsolidateApply))
	mux.HandleFunc("POST /admin/techniques/consolidate/{lane}/discard", s.adminOnly(s.handleConsolidateDiscard))
