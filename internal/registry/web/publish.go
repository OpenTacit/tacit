...
import (
	...
	"strings"
	"sync"
	"testing" // for the guard below: a test must not enrol against the real proxy
	"time"
...
func (s *Server) StartPublishing(handler http.Handler) {
	...
	// A test must not enrol against the proxy the project runs. DefaultIngress
	// names it, so any test that turns Global Access on and does not redirect
	// the address reaches production, takes a hostname, never comes back, and
	// leaves the operator an instance nobody can account for.
	//
	// Declaring the connection a test (config.PublishTest) makes those legible
	// and reclaimable; it does not make them right. A test that means to
	// exercise publishing points the address at its own ingress — every one
	// that does so today already sets it — so refusing the default here costs
	// nothing and closes the hole for tests nobody has written yet.
	if testing.Testing() && addr == config.DefaultIngress {
		s.pub.running = false
		s.pub.state = PublishState{LastError: "not publishing: a test may not enrol against " +
			config.DefaultIngress + " — set TACIT_PUBLISH_INGRESS to your own ingress"}
		s.pub.mu.Unlock()
		log.Printf("[publish] refused: a test tried to publish through the default ingress (%s). "+
			"Point TACIT_PUBLISH_INGRESS at a test ingress, or leave Global Access off.", config.DefaultIngress)
		return
	}
	...
	client := &ingress.Client{
		...
		// Whether this registry is somebody's or is exercising the proxy. It is
		// declared rather than guessed, because the proxy cannot tell a trial
		// run from a tenant's first day and the cost of guessing wrong is a
		// real organization's address being reissued.
		Test: s.cfg().PublishTest,
