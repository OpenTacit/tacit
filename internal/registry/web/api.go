...
func (s *Server) handleContribute(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		// A duplicate is not a malformed contribution. It is answered with the
		// technique the registry already holds, so the caller can offer it
		// instead of filing a second copy of it.
		var dup *contribute.DuplicateError
		if errors.As(err, &dup) {
			s.sendJSON(w, http.StatusConflict, map[string]any{
				...
				"id":         dup.Match.ID,
				"name":       dup.Match.Name,
				"status":     dup.Match.Status,
				"similarity": dup.Similarity,
				"hint":       "use it, revise it with /v1/techniques/revise, or resubmit with \"confirm_new\": true if it is a different move",
			})
			return
		}
		code := 400
