...
func (s *Server) pageReview(r *http.Request, user oidc.Claims) page {
	...
	b.WriteString(reviewNav(len(drafts), len(shadow), len(decayed), len(never), gate.auto))
	...
	b.WriteString(reviewShadow(shadow, insights.ShadowFits(events), gate))
...
func reviewNav(drafts, shadow, decayed, never int, autoShadow bool) string {
	blocking := drafts + decayed + never
	if !autoShadow {
		blocking += shadow
	}
	if blocking == 0 && shadow == 0 {
		return `<p class="empty">No items need review. Members can add drafts with <code>/tacit:contribute</code>, or you can run a suggestion search below.</p>`
	}
...
func (s *Server) reviewDrafts(drafts []models.Technique) string {
	...
	// The action bar sits ABOVE the table: bulk delete on the left (disabled
	// until something is checked, with a live count), suggest on the right.
	// Checkboxes reference the form by id, so the table stays outside any form
	// and row navigation keeps working. Deletion is confirmed client-side with
	// the count and re-checked server-side (drafts only).
	b.WriteString(`<div class="toolbar">` +
		`<form id="bulk-drafts" method="post" action="/admin/techniques/bulk-delete">` +
		`<button class="danger" type="submit" disabled ` +
		`title="Remove the selected drafts fully; reject keeps a record, delete keeps nothing">Delete selected</button></form>` +
		`<span class="sub" id="sel-count"></span>` +
		s.suggestForm() + `</div>`)
	...
	fmt.Fprintf(&b, `<div class="table-wrap"><table class="techniques data-table techniques-drafts"><thead><tr>`+
		`<th class="sel"><input type="checkbox" id="sel-all" aria-label="Select all drafts"></th>`+
		`<th>name</th><th>contributor</th><th>tags</th><th>added</th></tr></thead><tbody>%s</tbody></table></div>`, rows.String())

	// The toolbar renders ABOVE the table, so this must wait for the checkboxes
	// to exist — running it inline was the bug that left delete permanently
	// disabled.
	b.WriteString(`<script>document.addEventListener('DOMContentLoaded',function(){` +
		`var all=document.getElementById('sel-all');` +
		`var boxes=[].slice.call(document.querySelectorAll('input[form="bulk-drafts"][name="id"]'));` +
		`var f=document.getElementById('bulk-drafts');` +
		`if(!f||!boxes.length)return;` +
		`var btn=f.querySelector('button'),count=document.getElementById('sel-count');` +
		`function sync(){var n=boxes.filter(function(b){return b.checked;}).length;` +
		`btn.disabled=!n;count.textContent=n?n+' of '+boxes.length+' selected':'';}` +
		`boxes.forEach(function(b){b.addEventListener('change',sync);` +
		// The whole cell is the click target — a 2rem cell with a small box is
		// otherwise a row-navigation trap right next to the checkbox.
		`b.closest('td').addEventListener('click',function(e){` +
		`if(e.target!==b){b.checked=!b.checked;sync();}});});` +
		...
		`if(!confirm('Delete '+n+' draft'+(n===1?'':'s')+'? You cannot undo this.'))e.preventDefault();});` +
		`});</script>`)
