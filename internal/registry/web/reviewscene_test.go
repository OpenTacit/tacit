...
// ---- the vertical drawing below 760px ----

// narrowRule is one rule out of the Review picture's narrow block. The wide
// layout declares the same selectors with different coordinates, and reviewRule
// finds the first of them, so a test of the phone drawing has to say so.
func narrowRule(t *testing.T, sel string) string {
	t.Helper()
	block := appCSS[strings.Index(appCSS, "/* ---- the Review picture"):]
	block = block[:strings.Index(block, "/* ---- the Members picture")]
	block = block[strings.Index(block, "@media (max-width:"):]
	// Not the first match: .rev-g2 is the tail of .rev-g1,.rev-g2, and the
	// grouped rule holds what the two gates share rather than what either one
	// is. A selector reached over a comma is a different selector.
	for i, from := 0, 0; ; from = i + 1 {
		i = strings.Index(block[from:], sel+"{")
		if i < 0 {
			t.Fatalf("the narrow Review layout has no %s rule", sel)
		}
		i += from
		if i > 0 && block[i-1] == ',' {
			continue
...
// narrowBeam reads a beam out of the narrow block, in the scene's em.
func narrowBeam(t *testing.T, sel string) (x, y, length, ang float64) {
	t.Helper()
	m := regexp.MustCompile(`--x:(-?[0-9.]+)em;--y:(-?[0-9.]+)em;--len:([0-9.]+)em;--ang:(-?[0-9.]+)deg`).
		FindStringSubmatch(narrowRule(t, sel))
	if m == nil {
		t.Fatalf("cannot read %s's geometry from the narrow layout", sel)
	}
	v := make([]float64, 4)
	for i := range v {
		v[i], _ = strconv.ParseFloat(m[i+1], 64)
...
// nodeBox is where a stack is actually DRAWN, which is not where it is placed.
// A node is a square box turned on two axes: rotateZ puts its diagonal across
// the page, rotateX flattens what that leaves, and the whole thing is scaled
// about its own middle. So the middle is the one point the declared left and top
// give directly, and the drawn half-spans come off the box's own size and
// angles — all of which the stylesheet states, and none of which this test is
// entitled to guess at.
func nodeBox(t *testing.T, sel string) (cx, cy, halfW, halfH float64) {
	t.Helper()
	rule := narrowRule(t, sel)
	m := regexp.MustCompile(`left:([0-9.]+)em;top:([0-9.]+)em`).FindStringSubmatch(rule)
	if m == nil {
		t.Fatalf("%s is not placed in scene em: %s", sel, rule)
	}
	...
	scale := 1.0
	if m = regexp.MustCompile(`scale\(([0-9.]+)\)`).FindStringSubmatch(rule); m != nil {
		scale, _ = strconv.ParseFloat(m[1], 64)
	}
	shared := setRule(t, ".dgm-node")
	m = regexp.MustCompile(`width:([0-9.]+)em`).FindStringSubmatch(shared)
	if m == nil {
		t.Fatalf("the shared node states no size: %s", shared)
	}
	side, _ := strconv.ParseFloat(m[1], 64)
	m = regexp.MustCompile(`rotateX\(([0-9.]+)deg\)`).FindStringSubmatch(shared)
	if m == nil {
		t.Fatalf("the shared node states no tilt: %s", shared)
	}
	tilt, _ := strconv.ParseFloat(m[1], 64)
	diag := side * math.Sqrt2 / 2
	return left + side/2, top + side/2, diag * scale, diag * math.Cos(tilt*math.Pi/180) * scale
}

// narrowPort is a gate's drawn box in the narrow layout: the top is shared by
// both and the left is its own.
func narrowPort(t *testing.T, sel string) (left, top, width, height float64) {
	t.Helper()
	shared := narrowRule(t, ".rev-g1,.rev-g2")
	read := func(rule, name string) float64 {
		m := regexp.MustCompile(name + `:([0-9.]+)em`).FindStringSubmatch(rule)
		if m == nil {
			t.Fatalf("%s states no %s: %s", sel, name, rule)
		}
		v, _ := strconv.ParseFloat(m[1], 64)
		return v
	}
	m := regexp.MustCompile(`left:([0-9.]+)em`).FindStringSubmatch(narrowRule(t, sel))
	if m == nil {
		t.Fatalf("%s is not placed across the plate", sel)
	}
	left, _ = strconv.ParseFloat(m[1], 64)
	return left, read(shared, "top"), read(shared, "--port-w"), read(shared, "--port-h")
}

// EVERY LANE REACHES WHAT IT JOINS. Stood on end, the two ways in run four
// times as far between stack and gate as they do across the wide plate, and the
// beams were left at their old length: each stack hung on nothing, and the line
// into each gate began in mid-air a third of the way down. A flow drawn in
// pieces is not a flow.
func TestNarrowReviewLanesReachWhatTheyJoin(t *testing.T) {
	for _, lane := range []struct{ node, beam, gate string }{
		{".rev-a", ".rev-to-g1", ".rev-g1"},
		{".rev-b", ".rev-to-g2", ".rev-g2"},
	} {
		cx, cy, _, halfH := nodeBox(t, lane.node)
		x, y, length, ang := narrowBeam(t, lane.beam)
		_, top, _, _ := narrowPort(t, lane.gate)
		if ang != 90 {
			t.Errorf("%s leaves %s at %.1f°; the narrow lanes run straight down", lane.beam, lane.node, ang)
		}
		if math.Abs(x-cx) > 0.05 {
			t.Errorf("%s runs at %.2fem but %s stands at %.2fem: the lane is not under its stack",
				lane.beam, x, lane.node, cx)
		}
		if bottom := cy + halfH; math.Abs(y-bottom) > 0.1 {
			t.Errorf("%s starts %.2fem below %s, which is drawn down to %.2fem: the stack hangs on nothing",
				lane.beam, y, lane.node, bottom)
		}
		if math.Abs(y+length-top) > 0.05 {
			t.Errorf("%s ends at %.2fem and %s opens at %.2fem: the line into the gate is short",
				lane.beam, y+length, lane.gate, top)
		}
	}
}

// And the merge lands. Both arms leave their gate and must reach the serving
// stack — under its rim, which is drawn over them, rather than stopping an em
// above it in clear air.
func TestNarrowReviewMergeLandsOnTheServingStack(t *testing.T) {
	cx, cy, halfW, halfH := nodeBox(t, ".rev-c")
	for _, arm := range []struct{ beam, gate string }{
		{".rev-serve-up", ".rev-g1"}, {".rev-serve-down", ".rev-g2"},
	} {
		x, y, length, ang := narrowBeam(t, arm.beam)
		_, top, _, height := narrowPort(t, arm.gate)
		if math.Abs(y-(top+height)) > 0.05 {
			t.Errorf("%s starts at %.2fem and %s ends at %.2fem: the arm does not leave the gate",
				arm.beam, y, arm.gate, top+height)
		}
		ex := x + length*math.Cos(ang*math.Pi/180)
		ey := y + length*math.Sin(ang*math.Pi/180)
		if math.Abs(ex-cx) > halfW || ey < cy-halfH || ey > cy {
			t.Errorf("%s ends at (%.2f, %.2f)em; the serving stack is drawn from %.2f to %.2f across "+
				"and %.2f to %.2f down, and an arm has to reach its upper half",
				arm.beam, ex, ey, cx-halfW, cx+halfW, cy-halfH, cy)
		}
	}
}
