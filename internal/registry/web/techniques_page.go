...
func (s *Server) pageIndex(r *http.Request, user oidc.Claims) page {
	...
	// Techniques that say the same thing are a question about the library, so
	// the library is where a reader stands when they think to ask it — the same
	// reason tag consolidation sits on the Tags page. The panel itself stays in
	// one place (the review lane, where retiring something is decided); this is
	// the way to it from here.
	duplicates := ""
	if !archive {
		duplicates = ` · <a href="#consolidate">find duplicates</a>`
	}
	content := fmt.Sprintf(`<div class="page-head"><p class="sub">%s</p></div>`+
		`%s`+
		`%s`+
		`<div class="sub techniques-subline">%d reviewed technique%s%s%s · <span class="muted">select a row for details</span></div>`+
		`%s`+
		`<div class="table-wrap"><table class="`+tableClass+`"><thead><tr><th>name</th><th>source</th><th>tags</th>`+actionHead+`</tr></thead>
<tbody>%s</tbody></table></div>`,
		blurb, contributed, bar, shown, plural(shown), orgNote, duplicates, noteHTML, body)
	if grouped {
		content += techniquesGroupScript
	}
	// The library's own duplicates, decided here. Not on the archive: nothing is
	// decided by consolidating what has already stopped serving.
	if !archive {
		content += s.consolidatePanel("playbook")
	}
