...
// bulkDraftActions is what the review lane's action bar may do to a checked
// set of drafts: the same three decisions the draft page posts one at a time,
// plus the delete that has no single-draft twin — reject keeps a record, delete
// keeps nothing, and only a run of misses is ever worth purging. The value is
// the status the draft lands in; delete lands nowhere, so its value is empty.
var bulkDraftActions = map[string]string{
	"promote":   "stable",
	"to-shadow": "shadow",
	"reject":    "retired",
	"delete":    "",
...
// handleBulkDraftAction applies one of those decisions to every checked draft,
// so a suggestion run is cleared from the queue without opening forty pages.
// Same session gate as the single-draft actions, and the draft-only rule is
// enforced per id here, not just in the UI: a drive-by POST cannot retire or
// delete a stable technique's evidence.
func (s *Server) handleBulkDraftAction(w http.ResponseWriter, r *http.Request) {
	if !s.signedInOrRedirect(w, r, "/review") {
		return
	}
	to, ok := bulkDraftActions[r.PathValue("action")]
	if !ok {
		s.sendError(w, 404, "unknown action")
		return
	}
	...
	for _, id := range r.Form["id"] {
		...
		if err != nil || !found || technique.Status != "draft" {
			continue
		}
		if to == "" {
		...
		_, _, _ = contribute.Promote(s.Store, id, to, s.Embedder)
	}
