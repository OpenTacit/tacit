...
import (
	...
	"path/filepath"
	"strings"
	// testing, in a package that ships in the binary, for exactly one thing:
	// PublishTest's default. testing.Testing() is the supported way to ask
	// whether this process was built by `go test`, it registers no flags, and
	// it costs about twelve kilobytes. The alternative is sniffing os.Args for
	// a ".test" suffix, which is a guess where this is a fact.
	"testing"
...
type Config struct {
	...
	PublishTLS     bool
	// PublishTest tells the proxy this registry is a test rather than an
	// organization's (TACIT_PUBLISH_TEST). Turn it on for a scratch instance, a
	// CI run, or anything else exercising the ingress: the proxy's operator can
	// then tell those apart from tenants, and may reclaim the hostname once it
	// stops connecting. Off means "I am somebody's registry", which is the
	// answer a real one must give by default.
	//
	// Under `go test` it defaults ON, because DefaultIngress names the proxy the
	// project actually runs: a test that enables publishing and forgets to point
	// the address somewhere harmless enrols against production, and the fleet
	// fills with hostnames nobody can account for. That default is not a guess
	// about what the registry is FOR — it is a fact about the process it is
	// running in, and there is no such thing as an organization's registry
	// inside a test binary. TACIT_PUBLISH_TEST=0 still overrides it, for the one
	// case that needs to prove what a non-test registry declares.
	PublishTest bool
...
const (
	...
	// DupBlockFloor shortlists the techniques worth asking about. Sameness is
	// decided on the recipe (embed.MoveText), which no stored vector carries, so
	// deciding it for the whole corpus would mean embedding the whole corpus on
	// every proposal. The stored vector narrows the field first.
	//
	// Measured over a 174-technique corpus: of the 360 pairs the recipe calls
	// duplicates at DupThreshold, the weakest scores 0.615 on the stored vector.
	// A floor of 0.45 therefore keeps all of them, with margin, and cuts the set
	// a recipe is computed for from 174 to about 33.
	DupBlockFloor        = 0.45
...
func Load() Config {
	...
	return Config{
		...
		PublishTest:           env([]string{"TACIT_PUBLISH_TEST"}, boolStr(testing.Testing())) == "1",
