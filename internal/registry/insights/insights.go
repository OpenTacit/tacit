...
// MergedInto maps a consolidated technique to the one that survived it, built
// from the techniques the caller already holds. Folds here redirect through it
// so the review lane shows the same evidence RetireStaleShadow and
// AutoPromoteShadow act on; a lane reading different numbers from the gate it
// describes is worse than no readout.
func MergedInto(techniques []models.Technique) map[string]string {
...
func ShadowFits(events []models.FeedbackEvent, into map[string]string) map[string]ShadowFit {
