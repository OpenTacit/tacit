...
func RecomputeOutcomes(st storage.Store) (int, error) {
	...
	// A consolidated technique's events count toward the one that survived it,
	// on the same in-memory terms as the cohort recovery above (merged.go).
	into, err := mergedInto(st)
	if err != nil {
		return 0, err
	}
	...
	for _, e := range events {
		...
		id := redirect(into, e.TechniqueID)
...
func shadowFitCounts(st storage.Store) (map[string]shadowFit, error) {
	...
	// The fit verdicts of a consolidated technique belong to the survivor. They
	// are what RetireStaleShadow and AutoPromoteShadow decide on, so a merge
	// that dropped them would hand the survivor a fresh start it did not earn —
	// including a clean slate on declines its own group had already collected.
	into, err := mergedInto(st)
	if err != nil {
		return nil, err
	}
