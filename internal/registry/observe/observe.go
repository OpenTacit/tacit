...
// recordRestatements writes one event per proposal the novelty gate turned
// away, against the technique it restated.
//
// A pass that distils the same move out of usage for the ninth time has learned
// something — that this move keeps happening — and dropping the proposal used
// to drop that too. The event is where it survives: on the technique the
// restatement was about, in the view an operator already reads.
//
// Best-effort. A pass that filed its techniques must not fail because a note
// about the ones it did not file could not be written.
func recordRestatements(st observationStore, dups []contribute.Duplicate) {
	for _, d := range dups {
		_, _ = st.AppendLifecycleEvent(models.LifecycleEvent{
			EventID: contracts.DeterministicLifecycleEventID(
				d.Match.ID, "restated", d.Proposal.CreatedAt+d.Proposal.ID),
			TechniqueID:   d.Match.ID,
			TechniqueName: d.Match.Name,
			Kind:          "restated",
			Provenance:    d.Proposal.Provenance,
			Reason:        "usage distilled this move again, as " + d.Proposal.Name,
			CreatedAt:     d.Proposal.CreatedAt,
		})
	}
}
...
func Run(st observationStore, embedder embed.Embedder, model Completer, cfg Config) ([]models.Technique, error) {
	...
	recordRestatements(st, landing.Duplicates)
	return created, nil
}
