...
func Run(t *testing.T, open Factory) {
	...
	t.Run("MergedIntoRoundTrips", func(t *testing.T) { mergedIntoRoundTrips(t, open(t)) })
...
// A technique consolidated into another records which one, and every backend
// has to keep it: the redirect that makes a merge additive is built by reading
// this field back, so a backend that drops it silently un-merges the group and
// the survivor loses the evidence it was kept for.
func mergedIntoRoundTrips(t *testing.T, st storage.Store) {
	keeper := technique("keeper")
	must(t, st.UpsertTechnique(keeper))

	merged := technique("merged")
	merged.MergedInto = keeper.ID
	must(t, st.UpsertTechnique(merged))

	got, ok, err := st.GetTechnique(merged.ID)
	must(t, err)
	if !ok || got.MergedInto != keeper.ID {
		t.Fatalf("merged_into did not round-trip: got %q want %q", got.MergedInto, keeper.ID)
	}
	// It must also come back through a list, which is how the redirect is built.
	all, err := st.ListTechniques(nil, 0)
	must(t, err)
	var seen bool
	for _, c := range all {
		if c.ID == merged.ID {
			seen = c.MergedInto == keeper.ID
		}
		if c.ID == keeper.ID && c.MergedInto != "" {
			t.Fatalf("the survivor came back pointing at %q", c.MergedInto)
		}
	}
	if !seen {
		t.Fatal("merged_into is stored but not listed, so no redirect can be built from it")
	}
}
