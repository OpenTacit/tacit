...
// bodyDoing is body with a move of its own. Sameness is decided on the recipe,
// so a test that wants a second, unrelated draft has to say what it does —
// reusing body() twice now files one technique and a refusal.
func bodyDoing(name, recipe string) map[string]any {
	b := body(name)
	b["recipe"] = recipe
	return b
}
...
func TestUniqueIDNeverOverwrites(t *testing.T) {
	...
	// A second filing of the same move is refused (below); this is the member
	// answering that refusal — they read the match and meant it. The suffix is
	// what stops the confirmed one landing on top of the first.
	confirmed := body("Same Name")
	confirmed["confirm_new"] = true
	second, err := Create(st, confirmed, embedder)
	if err != nil {
		t.Fatalf("a confirmed contribution was refused: %v", err)
	}
...
// UniqueID minting slug-2 is what let the same technique into the registry
// twice under one name, filed by a member nobody told. A contribution that
// restates something already here is now answered with what it restates.
func TestAContributionThatRestatesAnExistingTechniqueIsRefused(t *testing.T) {
	st, embedder := setup(t)
	first, err := Create(st, body("Query the warehouse"), embedder)
	if err != nil {
		t.Fatalf("first contribution: %v", err)
	}

	_, err = Create(st, body("Ask the warehouse instead"), embedder)
	var dup *DuplicateError
	if !errors.As(err, &dup) {
		t.Fatalf("a restatement was filed as a new technique: %v", err)
	}
	if dup.Match.ID != first.ID {
		t.Errorf("refused against %q, expected %q", dup.Match.ID, first.ID)
	}
	if dup.Similarity < 0.9 {
		t.Errorf("refused on a similarity of %.3f, below the threshold it claims", dup.Similarity)
	}
	// The member is told what to use instead — a refusal naming nothing is
	// worse than the duplicate it prevents.
	if !strings.Contains(dup.Error(), first.Name) || !strings.Contains(dup.Error(), first.ID) {
		t.Errorf("the refusal does not name the technique it points at: %s", dup.Error())
	}
	// And nothing was left behind by the refusal.
	all, _ := st.ListTechniques(nil, 0)
	if len(all) != 1 {
		t.Errorf("a refused contribution left %d techniques behind, expected 1", len(all))
	}
}

// Sameness is decided on the recipe, because that is the move. Two techniques
// that apply in the same situation and tell you to do different things are two
// techniques.
func TestADifferentMoveInTheSameSituationIsNotADuplicate(t *testing.T) {
	st, embedder := setup(t)
	if _, err := Create(st, body("Query the warehouse"), embedder); err != nil {
		t.Fatal(err)
	}
	other := body("Query the warehouse")
	other["recipe"] = "export the table to CSV and attach it to the session"
	if _, err := Create(st, other, embedder); err != nil {
		var dup *DuplicateError
		if errors.As(err, &dup) {
			t.Fatalf("a different move was refused as a duplicate of %q", dup.Match.ID)
		}
		t.Fatal(err)
	}
}
...
func TestRevertToRejectsBadTargets(t *testing.T) {
	...
	draft, err := Create(st, bodyDoing("Draft move", "open the dashboard and read the funnel"), embedder)
	if err != nil {
		t.Fatal(err)
	}
...
func TestReviseValidation(t *testing.T) {
	...
	draft, _ := Create(st, bodyDoing("Held out", "ask the owner before changing the schema"), embedder)
