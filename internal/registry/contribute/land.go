...
type Landing struct {
	...
	matcher    *embed.Matcher

	// Duplicates is what the pass turned away, and what it turned them away
	// for. A pass that keeps proposing techniques the registry already has is
	// saying something about the pass; a caller that never looks cannot hear it.
	Duplicates []Duplicate
}

// Duplicate is one proposal that was not filed, beside the technique it
// restates.
type Duplicate struct {
	Proposal models.Technique
...
func NewLanding(st landingStore, embedder embed.Embedder, existing []models.Technique, status, provenance string) *Landing {
	return &Landing{
		...
		now: models.Now(), existing: existing, matcher: embed.NewMatcher(embedder),
	}
}
...
func (l *Landing) LandDraft(p ProposedTechnique) (models.Technique, bool, error) {
	...
	if held, taken, err := l.store.GetTechnique(technique.ID); err != nil {
		return models.Technique{}, false, err
	} else if taken {
		// Same slug = same move; a rephrase is not a new technique. It is also
		// a duplicate, found a cheaper way than the matcher finds them, and it
		// is recorded like one — a restatement that happens to share a name
		// tells you no less than one that does not.
		l.Duplicates = append(l.Duplicates, Duplicate{Proposal: technique, Match: held})
		return models.Technique{}, false, nil
	}
	vec := l.embedder.Embed([]string{embed.TechniqueText(technique)})[0]
	if match, _, dup := l.matcher.Match(technique, vec, l.existing); dup {
		// Already here under another form of words. Dropping is right for a
		// machine lane — nobody is waiting on an answer — but the proposal was
		// evidence that this move matters, and recording that is what turns a
		// discarded restatement into a number on the technique it restates.
		l.Duplicates = append(l.Duplicates, Duplicate{Proposal: technique, Match: match})
		return models.Technique{}, false, nil
	}
