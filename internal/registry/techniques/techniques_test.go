...
func TestOrgTechniqueFrontmatterDetail(t *testing.T) {
	technique, err := ParseFile(filepath.Join(repoTechniques(t), "use-internal-data-connector.md"))
	if err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(technique.AppliesWhen, "manually pasted tabular data") {
		t.Fatalf("folded scalar wrong: %q", technique.AppliesWhen)
	}
	if !strings.Contains(technique.Recipe, "@warehouse query") {
