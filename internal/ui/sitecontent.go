...
// Where the masthead's navigation goes. Every one of these is the repository,
// because the repository is the only place the project publishes anything: the
// apex serves this page and nothing else (internal/ingress/site.go answers 404 to
// every other path), and there is no docs site, no blog and no release feed under
// tacit.zone to point at. When one of those exists, its href replaces the line
// here and nothing else on the page changes.
//
// NOTE they are all dead until the repository is public — github.com/opentacit/tacit
...
// There is no "Blog" either, and there was until the announcement it pointed at
// stopped being in this repository. A masthead link to a document nobody can open
// is the navigation version of an empty panel: it promises a destination and
// spends a stranger's click proving there isn't one. It comes back when there is
