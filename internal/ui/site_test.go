...
// The masthead's destinations all live in the repository, which does not answer
// to a stranger yet. This is the reminder, not a rule: it fails the moment
// somebody points the navigation at a host that is not GitHub, at which point the
// note in site.go about the repository having to be public first is stale and
// should go with it.
func TestMastheadDestinationsAreAllTheRepository(t *testing.T) {
	for _, n := range append(siteNav, struct{ Label, Href string }{"repository", SiteRepoHref}) {
		if !strings.HasPrefix(n.Href, SiteRepoHref) {
			t.Errorf("%s points at %s, which is outside the repository — the note in site.go "+
				"about publishing the repository before the page is now out of date", n.Label, n.Href)
		}
	}
